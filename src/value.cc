...
Value Value::clone() const {
	switch (this->type()) {
		...
		break;
		case Type::RANGE :
			return Value(boost::get<RangePtr>(this->value).clone());
		break;
		case Type::VECTOR :
			return Value(boost::get<VectorType>(this->value).clone());
		break;
		case Type::EMBEDDED_VECTOR :
			return Value(boost::get<EmbeddedVectorType>(this->value).clone());
		break;
		case Type::FUNCTION :
	...
	for (size_t i=0;i<matrixvec.size();i++) {
		...
		for (size_t j=0;j<matrixvec[i].toVector().size();j++) {
			if (matrixvec[i].toVector()[j].type() != Value::Type::NUMBER ||
			    vectorvec[j].type() != Value::Type::NUMBER) {
				return Value::undef();
			}
...
Value Value::operator*(const Value &v) const
{
	if (this->type() == Type::NUMBER) {
		...
		if (v.type() == Type::NUMBER) {
			...
			if (vec1.empty() || vec2.empty()) return Value::undef();
				...
				if (vec2[0].type() == Type::NUMBER) {
					...
					if (vec1[0].toVector().size() == vec2.size()) {
						...
						for (const auto &srcrow : vec1) {
							...
							if (srcrowvec.size() != vec2.size()) return Value::undef();
